...
CREATE FUNCTION public.get_all_apuestas() RETURNS TABLE(idapuestas integer, idusuario integer, cantidadapostada numeric, fechaapuesta date, idestadoapuesta integer, idpartido integer, estadoapuesta character varying, usuario character varying, fechapartido date)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        a.idapuestas AS idApuestas,
        a.idusuario AS idUsuario,
        a.cantidadapostada AS cantidadApostada,
        a.fechaapuesta AS fechaApuesta,
        a.idestadoapuesta AS idEstadoApuesta,
        a.idpartido AS idPartido,
        e.estadodeapuesta AS estadoApuesta,
        u.username AS usuario,
        p.fechapartido AS fechaPartido
    FROM apuestas a
    JOIN estadoapuesta e ON a.idestadoapuesta = e.idestadoapuesta
    JOIN usuarios u ON a.idusuario = u.idusuario
    JOIN partido p ON a.idpartido = p.idpartido;
END;
$$;
...
CREATE FUNCTION public.get_apuesta_by_id(apuestaid integer) RETURNS TABLE(idapuestas integer, idusuario integer, cantidadapostada numeric, fechaapuesta date, idestadoapuesta integer, idpartido integer, estadoapuesta character varying, usuario character varying, fechapartido date)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    ...
    JOIN usuarios u ON a.idusuario = u.idusuario
    JOIN partido p ON a.idpartido = p.idpartido
    WHERE a.idapuestas = apuestaId;
END;
$$;


ALTER FUNCTION public.get_apuesta_by_id(apuestaid integer) OWNER TO postgres;
